...
d <- read_excel("../ACCESS/20220328_MOBELS.xlsx", sheet = "D-Loop", na = "NA")
str(d)
...
d[d$Responsable_Dloop %in% "0", "Responsable_Dloop"] <- NA
...
d[d$No_run_F %in% "0", "No_run_F"] <- NA  # NAs instead of 0
table(d$No_run_F, useNA = 'ifany')

table(d$No_run_R)
d[d$No_run_R %in% "0", "No_run_R"] <- NA  # NAs instead of 0
...
d[d$No_plaque_F %in% "0", "No_plaque_F"] <- NA  # NAs instead of 0
...
table(d$No_plaque_R)
d[d$No_plaque_R %in% "0", "No_plaque_R"] <- NA  # NAs instead of 0
...
d[d$No_puits_F %in% "0", "No_puits_F"] <- NA  # NAs instead of 0
table(d$No_puits_F, useNA = 'ifany')

table(d$No_puits_R, useNA = 'ifany')
d[d$No_puits_R %in% "0", "No_puits_R"] <- NA  # NAs instead of 0
table(d$No_puits_R, useNA = 'ifany')
...
d[d$Notes %in% "0", "Notes"] <- NA  # NAs instead of 0
