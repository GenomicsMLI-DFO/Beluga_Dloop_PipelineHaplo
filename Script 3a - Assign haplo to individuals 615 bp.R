...
# Assign haplotypes (HL) to specimens
# 
# Benjamin Hornoy
#
#
...
# Data --------------------------------------------------------------------
data <- read.csv("Sequences_Dloop615_n3314.csv")
str(data)
...
lib <- read.csv('librairie_137_haplotypes615.csv') # most recent haplotype library
...
seq_start <- as.numeric(unlist(strsplit(min_seq$Bornes.sequence.minimale[1], " - "))[1])  # start of minimal sequence
seq_stop <- as.numeric(unlist(strsplit(min_seq$Bornes.sequence.minimale[1], " - "))[2])  # end of minimal sequence
...
for (i in 1:length(sequences)){
    if(data$seq_utilisable[i] == "yes" & data$Qualite_sequence[i]==1) {
...
write.csv(data2, "Dloop_haplo615_n3314.csv", row.names=F)
