...
# Data --------------------------------------------------------------------
data <- read.table("Sequences_Dloop615_n3283.txt", header = T)
str(data)
colnames(data)[2] <- "seq"
...
dna <- fasta2DNAbin("Beluga_615bp_n3283.fasta")
...
seq_stop <- as.numeric(tail(snpPos, 1))  # position of last polymorphic site (584)
...
write.csv(data2, "Sequences_Dloop615_n3283.csv", row.names = F)
...
# Code above used to create new 'official' HL library
...
data2[substr(data2$seq, seq_start, seq_stop)==substr(lib_fin[lib_fin$hapl=="HL104","seq"], seq_start, seq_stop),]

# HL126 also found for S_20_03546 with A instead of R
data2[substr(data2$seq, seq_start, seq_stop)==substr(lib_fin[lib_fin$hapl=="HL126","seq"], seq_start, seq_stop),]
